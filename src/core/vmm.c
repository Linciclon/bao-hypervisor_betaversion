...
void           vmm_init() {
    ...
    if(cpu.id == CPU_MASTER) {
        iommu_init();
        vmass_npages = ALIGN(sizeof(struct vm_assignment) * vm_config_ptr->vmlist_size, PAGE_SIZE) / PAGE_SIZE;
        vm_assign = mem_alloc_page(vmass_npages, SEC_HYP_GLOBAL, false);
        if(vm_assign == NULL)
            ERROR("cant allocate vm assignemnt pages");
    ...
    for(int i = 0; i < vm_config_ptr->vmlist_size && !assigned; i++) {
        if(vm_config_ptr->vmlist[i].cpu_affinity & (1UL << cpu.id)) {
            ...
            if(!vm_assign[i].master) {
                ...
                assigned = true;
                vm_id = i;
            } else if(vm_assign[i].ncpus < vm_config_ptr->vmlist[i].platform.cpu_num) {
                assigned = true;
                vm_assign[i].ncpus++;
                vm_assign[i].cpus |= (1UL << cpu.id);
                vm_id = i;
    ...
    if(assigned == false) {
        for(int i = 0; i < vm_config_ptr->vmlist_size && !assigned; i++) {
            spin_lock(&vm_assign[i].lock);
            if(vm_assign[i].ncpus < vm_config_ptr->vmlist[i].platform.cpu_num) {
                if(!vm_assign[i].master) {
                    vm_assign[i].master = true;
                    vm_assign[i].ncpus++;
                    master = true;
    ...
    if(assigned) {
        ...
        if(master) {
            ...
            void*  va = mem_alloc_vpage(&cpu.as, SEC_HYP_VM, (void*)BAO_VM_BASE, vm_npages);
            mem_map(&cpu.as, va, NULL, vm_npages, PTE_HYP_FLAGS);
            memset(va, 0, vm_npages * PAGE_SIZE);
            fence_ord_write();
            vm_assign[vm_id].vm_shared_table = *pt_get_pte(&cpu.as.pt, 0, (void*)BAO_VM_BASE);
        } else {
