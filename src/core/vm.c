...
void vm_vcpu_init(vm_t* vm, const vm_config_t* config) {
    ...
    vcpu->regs = (struct arch_regs*)(vcpu->stack + sizeof(vcpu->stack) - sizeof(*vcpu->regs));
    vcpu_arch_init(vcpu, vm);
    vcpu_arch_reset(vcpu, config->entry);
    list_push(&vm->vcpu_list, &vcpu->node);
}
static void vm_copy_img_to_rgn(vm_t* vm, const vm_config_t* config, struct mem_region* reg) {
...
static void vm_map_img_rgn_inplace(vm_t* vm, const vm_config_t* config, struct mem_region* reg) {
    /* mem region pages before the img */
    size_t n_before = (config->image.base_addr - reg->base) / PAGE_SIZE;
    /* pages after the img */
    size_t n_aft = ((reg->base + reg->size) - (config->image.base_addr + config->image.size)) / PAGE_SIZE;
    ...
    if(all_clrs(vm->as.colors)) {
        /* map img in place */
        mem_map(&vm->as, va + n_before * PAGE_SIZE, &pa_img, n_img, PTE_VM_FLAGS);
        /* we are mapping in place, config is already reserved */
    } else {
        /* recolour img */
        mem_map_reclr(&vm->as, va + n_before * PAGE_SIZE, &pa_img, n_img, PTE_VM_FLAGS);
        /* TODO: reserve phys mem? */
    }
    /* map pages after img */
    mem_map(&vm->as, va + (n_before + n_img) * PAGE_SIZE, NULL, n_aft, PTE_VM_FLAGS);
}
static void vm_map_img_rgn(vm_t* vm, const vm_config_t* config, struct mem_region* reg) {
    if(reg->place_phys) {
...
static void vm_init_mem_regions(vm_t* vm, const vm_config_t* config) {
    for(int i = 0; i < config->platform.region_num; i++) {
        ...
        int img_is_in_rgn = range_in_range(config->image.base_addr, config->image.size, reg->base, reg->size);
        if(img_is_in_rgn) {
...
__attribute__((weak)) uint64_t vm_translate_to_pcpu_mask(vm_t* vm, uint64_t mask, size_t len) {
    uint64_t pmask = 0;
    int      shift;
    for(int i = 0; i < len; i++) {
        if((mask & (1ULL << i)) && ((shift = vm_translate_to_pcpuid(vm, i)) >= 0)) {
            pmask |= (1ULL << shift);
        }
    }
    return pmask;
}
__attribute__((weak)) uint64_t vm_translate_to_vcpu_mask(vm_t* vm, uint64_t mask, size_t len) {
    uint64_t pmask = 0;
    int      shift;
    for(int i = 0; i < len; i++) {
        if((mask & (1ULL << i)) && ((shift = vm_translate_to_vcpuid(vm, i)) >= 0)) {
            pmask |= (1ULL << shift);
        }
    }
    return pmask;
