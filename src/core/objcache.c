...
static slab_t* slab_create(objcache_t* oc, enum AS_SEC sec) {
    ...
    if(oc != NULL) {
        ...
        if(slab != NULL) {
            ...
            slab->header.objsize = ALIGN((sizeof(node_t) + oc->osize), sizeof(uint64_t));
            slab->header.objnum = (PAGE_SIZE - sizeof(((slab_t*)NULL)->header)) / slab->header.objsize;
...
static bool slab_free(slab_t* slab, void* obj) {
    ...
    if(slab != NULL) {
        if(((((uint64_t)slab) & ~(PAGE_SIZE - 1)) ==
            (((uint64_t)obj_addr) & ~(PAGE_SIZE - 1))) && // obj is part of slab
           ((((((uint64_t)obj_addr) & (PAGE_SIZE - 1)) - sizeof(slab->header)) % slab->header.objsize) ==
            0) &&                                          // is aligned to object in slab
           (*((node_t*)(obj - sizeof(node_t))) == NULL)) { // the node is not currently in any slab
