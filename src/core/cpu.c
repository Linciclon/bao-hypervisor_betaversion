...
void                     cpu_init(uint64_t cpu_id, uint64_t load_addr) {
    ...
    if(cpu.id == CPU_MASTER) {
        ...
        objcache_init(&msg_cache, sizeof(cpu_msg_node_t), SEC_HYP_GLOBAL, false);
        ipi_cpumsg_handlers = &_ipi_cpumsg_handlers_start;
        ipi_cpumsg_handler_num = ((uint64_t)&_ipi_cpumsg_handlers_size) / sizeof(cpu_msg_handler_t);
...
bool cpu_get_msg(cpu_msg_t* msg) {
    ...
    if((node = (cpu_msg_node_t*)list_pop(&cpu.interface.event_list)) != NULL) {
        *msg = node->msg;
