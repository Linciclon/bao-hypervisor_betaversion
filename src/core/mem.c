...
#include <tlb.h>
#include <vm.h>
extern uint8_t _image_start, _image_end, _dmem_phys_beg, _dmem_beg, _cpu_private_beg, _cpu_private_end, _vm_beg,
    _vm_end, _config_start, _config_end;
...
section_t hyp_secs[] = {
    [SEC_HYP_GLOBAL] = {&_dmem_beg, &_cpu_private_beg - 1, true, SPINLOCK_INITVAL},
    [SEC_HYP_IMAGE] = {&_image_start, &_image_end - 1, true, SPINLOCK_INITVAL},
    [SEC_HYP_DEVICE] = {&_image_end, &_dmem_beg - 1, true, SPINLOCK_INITVAL},
    [SEC_HYP_PRIVATE] = {&_cpu_private_beg, &_cpu_private_end - 1, false, SPINLOCK_INITVAL},
    [SEC_HYP_VM] = {&_vm_beg, &_vm_end - 1, true, SPINLOCK_INITVAL},
};
section_t vm_secs[] = {[SEC_VM_ANY] = {0x0, (void*)-1, false, SPINLOCK_INITVAL}};
struct {
    section_t* sec;
    size_t     sec_size;
} sections[] = {
...
static bool pp_alloc_clr(page_pool_t* pool, size_t n, uint64_t colors, ppages_t* ppages) {
...
static bool pp_alloc(page_pool_t* pool, size_t n, bool aligned, ppages_t* ppages) {
    ppages->colors = 0;
    ppages->size = 0;
    bool     ok = false;
    uint64_t base = -1;
    ...
    for(int i = 0; i < 2 && !ok; i++) {
        while(pool->free != 0) {
            int64_t bit = bitmap_find_consec(pool->bitmap, pool->size, curr, n, false);
            if(bit < 0) {
                /**
...
ppages_t mem_alloc_ppages(uint64_t colors, size_t n, bool aligned) {
    ...
    list_foreach(page_pool_list, page_pool_t, pool) {
        bool ok = (!all_clrs(colors) && !aligned) ? pp_alloc_clr(pool, n, colors, &pages)
...
static inline bool pte_allocable(addr_space_t* as, pte_t* pte, uint64_t lvl, uint64_t left, uint64_t addr) {
    return (lvl == (as->pt.dscr->lvls - 1)) ||
           (pt_lvl_terminal(&as->pt, lvl) && !pte_valid(pte) && (pt_lvlsize(&as->pt, lvl) <= (left * PAGE_SIZE)) &&
            ((addr % pt_lvlsize(&as->pt, lvl)) == 0));
}
static inline pte_t* mem_alloc_pt(addr_space_t* as, pte_t* parent, uint64_t lvl, uint64_t addr) {
...
static inline bool pt_pte_mappable(addr_space_t* as,
                                   pte_t*        pte,
                                   uint64_t      lvl,
                                   uint64_t      left,
                                   uint64_t      vaddr,
    ...
    return !pte_valid(pte) && (pt_lvlsize(&as->pt, lvl) <= (left * PAGE_SIZE)) &&
           (((uint64_t)vaddr % pt_lvlsize(&as->pt, lvl)) == 0) && ((paddr % pt_lvlsize(&as->pt, lvl)) == 0);
}
...
void* mem_alloc_vpage(addr_space_t* as, enum AS_SEC section, void* at, size_t n) {
    ...
    while(count < n && !failed) {
        ...
        while((entry < nentries) && (count < n) && !failed) {
            if(pte_check_rsw(pte, PTE_RSW_RSRV) || (pte_valid(pte) && !pte_table(&as->pt, pte, lvl))) {
                count = 0;
                vpage = NULL;
                if(at != NULL) {
                    failed = true;
    ...
    while(vaddr < top) {
        ...
        if(pte == NULL) {
            ...
            while((entry < nentries) && (vaddr < top)) {
                if(!pte_table(&as->pt, pte, lvl)) {
                    void* vpage_base = (void*)(((uint64_t)vaddr) & ~(lvlsz - 1));
                    ...
                    if(free_ppages) {
                        ...
                        ppages_t ppages = mem_ppages_get(paddr, lvlsz / PAGE_SIZE);
...
int mem_map(addr_space_t* as, void* va, ppages_t* ppages, size_t n, uint64_t flags) {
    size_t     count = 0;
    pte_t*     pte = NULL;
        ...
        while(count < n) {
            ...
            for(lvl = 0; lvl < as->pt.dscr->lvls; lvl++) {
                ...
                if(pt_lvl_terminal(&as->pt, lvl)) {
                    if(pt_pte_mappable(as, pte, lvl, n - count, (uint64_t)vaddr, ppages ? paddr : 0)) {
                        break;
                    } else if(!pte_valid(pte)) {
            ...
            uint64_t nentries = pt_nentries(&as->pt, lvl);
            uint64_t lvlsz = pt_lvlsize(&as->pt, lvl);
            while((entry < nentries) && (count < n) && (n - count >= lvlsz / PAGE_SIZE)) {
                if(ppages == NULL) {
                    ppages_t temp = mem_alloc_ppages(as->colors, lvlsz / PAGE_SIZE, true);
...
int mem_map_reclr(addr_space_t* as, void* va, ppages_t* ppages, size_t n, uint64_t flags) {
    if(ppages == NULL) {
     ...
     */
    uint64_t reclrd_num =
        n / (COLOR_NUM * COLOR_SIZE) * COLOR_SIZE * bitmap_count((bitmap_t)&as->colors, 0, COLOR_NUM, false);
    uint64_t clr_offset = (ppages->base / PAGE_SIZE) % (COLOR_NUM * COLOR_SIZE);
    for(int i = 0; i < (n % (COLOR_NUM * COLOR_SIZE)); i++) {
        if(!bitmap_get((bitmap_t)&as->colors, (i + clr_offset) / COLOR_SIZE % COLOR_NUM))
            reclrd_num++;
    }
    /**
    ...
    void*    reclrd_va_base = mem_alloc_vpage(&cpu.as, SEC_HYP_VM, NULL, reclrd_num);
     ...
     */
    ppages_t unused_pages = {.base = ppages->base, .size = reclrd_num, .colors = ~as->colors};
...
bool mem_are_ppages_reserved_in_pool(page_pool_t* ppool, ppages_t* ppages) {
    bool reserved = false;
    bool rgn_found = range_in_range(ppages->base, ppages->size * PAGE_SIZE, ppool->base, ppool->size * PAGE_SIZE);
    if(rgn_found) {
        ...
        size_t avlbl_contig_pp = bitmap_count_consecutive(ppool->bitmap, ppool->size, pageoff, numpages);
...
bool mem_are_ppages_reserved(ppages_t* ppages) {
    bool reserved = false;
    list_foreach(page_pool_list, page_pool_t, pool) {
        bool is_in_rgn = range_in_range(ppages->base, ppages->size * PAGE_SIZE, pool->base, pool->size * PAGE_SIZE);
        if(is_in_rgn) {
...
int mem_map_dev(addr_space_t* as, void* va, uint64_t base, size_t n) {
    ...
    return mem_map(as, va, &pages, n, as->type == AS_HYP ? PTE_HYP_DEV_FLAGS : PTE_VM_DEV_FLAGS);
}
...
bool root_pool_set_up_bitmap(uint64_t load_addr) {
    ...
    size_t   cpu_size = platform.cpu_num * (ALIGN(sizeof(struct cpu), PAGE_SIZE) + (PT_SIZE * (PT_LVLS - 1)));
    uint64_t bitmap_size = root_pool.size / (8 * PAGE_SIZE) + ((root_pool.size % (8 * PAGE_SIZE) != 0) ? 1 : 0);
    ...
    bitmap_t root_bitmap = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, NULL, bitmap_size);
    if(root_bitmap == NULL)
        return false;
...
bool pp_root_reserve_hyp_mem(uint64_t load_addr) {
    size_t   image_size = (size_t)(&_image_end - &_image_start);
    size_t   config_size = (size_t)(&_config_end - &_config_start);
    uint64_t cpu_size = platform.cpu_num * (ALIGN(sizeof(struct cpu), PAGE_SIZE) + (PT_SIZE * (PT_LVLS - 1)));
...
static bool pp_root_init(uint64_t load_addr, struct mem_region* root_region) {
    ...
    root_pool.size = root_region->size / PAGE_SIZE; /* TODO: what if not aligned? */
...
static void pp_init(page_pool_t* pool, uint64_t base, size_t size) {
    ...
    uint64_t bitmap_size = pool->size / (8 * PAGE_SIZE) + !!(pool->size % (8 * PAGE_SIZE) != 0);
    if(size <= bitmap_size)
        return;
    pages = mem_alloc_ppages(cpu.as.colors, bitmap_size, false);
    if(pages.size != bitmap_size)
        return;
    if((pool->bitmap = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, NULL, bitmap_size)) == NULL)
        return;
...
bool mem_reserve_config(uint64_t config_addr, page_pool_t* pool) {
    if(config_found)
        return true;
    bool cfg_in_pool = range_in_range(config_addr, vm_config_ptr->config_size, pool->base, pool->size * PAGE_SIZE);
    if(cfg_in_pool) {
        size_t   n_pg = NUM_PAGES(vm_config_ptr->config_size);
        ppages_t pp = mem_ppages_get(config_addr, n_pg);
        if(!mem_reserve_ppool_ppages(pool, &pp))
            return false;
        config_found = true;
    }
    return true;
...
bool mem_map_vm_config(uint64_t config_addr) {
    ...
    if(vm_config_ptr->config_header_size > PAGE_SIZE) {
        size_t n = NUM_PAGES(((uint64_t)vm_config_ptr->config_header_size) - PAGE_SIZE);
        void*  va = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, vm_config_ptr + PAGE_SIZE, n);
        if(va == NULL)
            return false;
...
void color_hypervisor(const uint64_t load_addr, const uint64_t config_addr) {
    ...
    size_t                image_size = (size_t)(&_image_end - &_image_start);
    size_t                cpu_boot_size = ALIGN(sizeof(cpu_t) + (PT_SIZE * (PT_LVLS - 1)), PAGE_SIZE);
    size_t                config_size = (size_t)(&_config_end - &_config_start);
    size_t   bitmap_size = (root_pool.size / (8 * PAGE_SIZE) + !!(root_pool.size % (8 * PAGE_SIZE) != 0)) * PAGE_SIZE;
    ...
    va = mem_alloc_vpage(&cpu_new->as, SEC_HYP_PRIVATE, (void*)BAO_CPU_BASE, NUM_PAGES(sizeof(cpu_t)));
    ...
    if(cpu.id == CPU_MASTER) {
        ...
        va = mem_alloc_vpage(&cpu_new->as, SEC_HYP_IMAGE, &_image_start, NUM_PAGES(image_size));
        if(va != &_image_start)
    ...
    p_interface = mem_ppages_get(p_intferface_addr, NUM_PAGES(sizeof(cpu_new->interface)));
    ...
    mem_map(&cpu_new->as, va, &p_interface, NUM_PAGES(sizeof(cpu_new->interface)), PTE_HYP_FLAGS);
    ...
    if(cpu.id == CPU_MASTER) {
        ...
        mem_map(&cpu_new->as, vm_config_ptr, &config_pages, config_size, PTE_HYP_FLAGS);
        /* Copy root pool bitmap */
        copy_space(root_pool.bitmap, bitmap_size, &p_bitmap);
        va = mem_alloc_vpage(&cpu_new->as, SEC_HYP_GLOBAL, (void*)root_pool.bitmap, NUM_PAGES(bitmap_size));
    ...
    if(cpu.id == CPU_MASTER) {
        ...
        p_bitmap = mem_ppages_get(load_addr + image_size + config_size + (cpu_boot_size * platform.cpu_num),
    ...
    p_cpu = mem_ppages_get(load_addr + image_size + config_size + (cpu_boot_size * cpu.id), cpu_boot_size / PAGE_SIZE);
...
void as_init(addr_space_t* as, enum AS_TYPE type, uint64_t id, void* root_pt, uint64_t colors) {
    as->type = type;
    ...
    if(root_pt == NULL) {
        ...
        root_pt = mem_alloc_page(n, type == AS_HYP || type == AS_HYP_CPY ? SEC_HYP_PRIVATE : SEC_HYP_VM, true);
...
void mem_init(uint64_t load_addr, uint64_t config_addr) {
    ...
    if(cpu.id == CPU_MASTER) {
        ...
        if(config_is_builtin()) {
            config_addr = (uint64_t)(&_config_start - &_image_start) + load_addr;
        }
