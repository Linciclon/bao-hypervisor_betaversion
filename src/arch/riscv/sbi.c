...
static inline struct sbiret sbi_ecall(long eid, long fid, long a0, long a1, long a2, long a3, long a4, long a5) {
...
struct sbiret sbi_remote_sfence_vma(const unsigned long hart_mask,
                                    ...
                                    unsigned long       size) {
    return sbi_ecall(SBI_EXTID_RFNC, SBI_REMOTE_SFENCE_VMA_FID, hart_mask, hart_mask_base, start_addr, size, 0, 0);
}
struct sbiret sbi_remote_hfence_gvma(const unsigned long hart_mask,
                                     unsigned long       hart_mask_base,
                                     unsigned long       start_addr,
                                     unsigned long       size) {
    return sbi_ecall(SBI_EXTID_RFNC, SBI_REMOTE_HFENCE_GVMA_FID, hart_mask, hart_mask_base, start_addr, size, 0, 0);
}
struct sbiret sbi_remote_hfence_gvma_vmid(const unsigned long hart_mask,
                                          ...
                                          unsigned long       size,
                                          unsigned long       vmid) {
    return sbi_ecall(SBI_EXTID_RFNC,
                     SBI_REMOTE_HFENCE_GVMA_VMID_FID,
                     hart_mask,
                     hart_mask_base,
                     start_addr,
                     size,
...
struct sbiret sbi_remote_hfence_vvma_asid(const unsigned long hart_mask,
                                          ...
                                          unsigned long       asid) {
    return sbi_ecall(SBI_EXTID_RFNC,
                     SBI_REMOTE_HFENCE_VVMA_ASID_FID,
                     hart_mask,
                     hart_mask_base,
                     start_addr,
                     size,
...
struct sbiret sbi_remote_hfence_vvma(const unsigned long hart_mask,
                                     ...
                                     unsigned long       start_addr,
                                     unsigned long       size) {
    return sbi_ecall(SBI_EXTID_RFNC, SBI_REMOTE_HFENCE_VVMA_FID, hart_mask, hart_mask_base, start_addr, size, 0, 0);
}
...
void sbi_msg_handler(uint32_t event, uint64_t data) {
    switch(event) {
        case SEND_IPI: CSRS(CSR_HVIP, HIP_VSSIP); break;
        case HART_START: {
        ...
        default: WARNING("unknown sbi msg"); break;
    }
}
...
struct sbiret sbi_base_handler(unsigned long fid) {
    ...
    switch(fid) {
        case SBI_GET_SBI_SPEC_VERSION_FID: ret.value = 2; break;
...
struct sbiret sbi_rfence_handler(unsigned long fid) {
    ...
    if((hart_mask_base != 0) &&
       ((hart_mask_base >= hart_mask_width) ||
        (bitmap_find_nth((bitmap_t)&hart_mask, hart_mask_width, 1, hart_mask_width - hart_mask_base, true) > 0))) {
        WARNING("sbi invalid hart_mask");
        return (struct sbiret) {SBI_ERR_INVALID_PARAM};
    }
    hart_mask = hart_mask >> hart_mask_base;
    unsigned long phart_mask = vm_translate_to_pcpu_mask(cpu.vcpu->vm, hart_mask, sizeof(hart_mask) * 8);
    switch(fid) {
        case SBI_REMOTE_FENCE_I_FID: ret = sbi_remote_fence_i(phart_mask, 0); break;
        case SBI_REMOTE_SFENCE_VMA_FID: ret = sbi_remote_hfence_vvma(phart_mask, 0, start_addr, size); break;
        case SBI_REMOTE_SFENCE_VMA_ASID_FID:
            ret = sbi_remote_hfence_vvma_asid(phart_mask, 0, start_addr, size, asid);
            break;
        default: ret.error = SBI_ERR_NOT_SUPPORTED;
    }
    return ret;
}
    ...
    if(vhart_id == cpu.vcpu->id) {
        ...
        if(vcpu == NULL) {
            ...
            if(vcpu->arch.sbi_ctx.state == STARTED) {
                ...
                fence_sync_write();
                cpu_msg_t msg = {.handler = SBI_MSG_ID, .event = HART_START, .data = 0xdeadbeef};
...
struct sbiret sbi_hsm_handler(unsigned long fid) {
    struct sbiret ret;
    switch(fid) {
        case SBI_HART_START_FID: ret = sbi_hsm_start_handler(); break;
        case SBI_HART_STATUS_FID: ret = sbi_hsm_status_handler(); break;
        default: ret.error = SBI_ERR_NOT_SUPPORTED;
    }
    return ret;
}
struct sbiret sbi_bao_handler(unsigned long fid) {
    ...
    switch(fid) {
        case HC_IPC: ret.error = ipc_hypercall(arg0, arg1, arg2); break;
        default: ret.error = -HC_E_INVAL_ID;
    }
    return ret;
}
void sbi_lgcy_sendipi_handler() {
    ...
    vm_readmem(cpu.vcpu->vm, &phart_mask, (uintptr_t)hart_mask, sizeof(unsigned long), false);
    phart_mask = vm_translate_to_pcpu_mask(cpu.vcpu->vm, phart_mask, sizeof(unsigned long));
    cpu_msg_t msg = {
        .handler = SBI_MSG_ID,
        .event = SEND_IPI,
    };
...
void sbi_lgcy_rfence_handler(unsigned long extid) {
    ...
    unsigned long phart_mask = 0;
    vm_readmem(cpu.vcpu->vm, &phart_mask, (uintptr_t)hart_mask, sizeof(unsigned long), false);
    phart_mask = vm_translate_to_pcpu_mask(cpu.vcpu->vm, (uint64_t)hart_mask, sizeof(unsigned long));
    unsigned long start_addr = vcpu_readreg(cpu.vcpu, REG_A2);
    unsigned long size = vcpu_readreg(cpu.vcpu, REG_A3);
    unsigned long asid = vcpu_readreg(cpu.vcpu, REG_A4);
    switch(extid) {
        case SBI_LGCY_EXTID_REMFENCEI: sbi_remote_fence_i(phart_mask, 0); break;
        case SBI_LGCY_EXTID_REMSFENCEVMA: sbi_remote_hfence_vvma(phart_mask, 0, start_addr, size); break;
        case SBI_LGCY_EXTID_REMSFENCEASID: sbi_remote_hfence_vvma_asid(phart_mask, 0, start_addr, size, asid); break;
    }
}
...
void sbi_lgcy_handler(unsigned long extid) {
    switch(extid) {
        case SBI_LGCY_EXTID_SETTIMER: (void)sbi_time_handler(SBI_SET_TIMER_FID); break;
        case SBI_LGCY_EXTID_SENDIPI: sbi_lgcy_sendipi_handler(); break;
        case SBI_LGCY_EXTID_REMFENCEI:
        case SBI_LGCY_EXTID_REMSFENCEVMA:
        case SBI_LGCY_EXTID_REMSFENCEASID: sbi_lgcy_rfence_handler(extid); break;
        case SBI_LGCY_EXTID_PUTCHAR: sbi_lgcy_putchar_handler(); break;
        default: WARNING("guest issued unsupported sbi legacy extension call (%d)", extid);
    }
}
size_t sbi_vs_handler() {
    ...
    if(extid < 16) {
        ...
        struct sbiret ret;
        switch(extid) {
            case SBI_EXTID_BASE: ret = sbi_base_handler(fid); break;
            case SBI_EXTID_TIME: ret = sbi_time_handler(fid); break;
            case SBI_EXTID_IPI: ret = sbi_ipi_handler(fid); break;
            case SBI_EXTID_RFNC: ret = sbi_rfence_handler(fid); break;
            case SBI_EXTID_HSM: ret = sbi_hsm_handler(fid); break;
            case SBI_EXTID_BAO: ret = sbi_bao_handler(fid); break;
            default:
