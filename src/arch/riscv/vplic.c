...
static void vplic_ipi_handler(uint32_t event, uint64_t data) {
    switch(event) {
        case UPDATE_HART_LINE: vplic_update_hart_line(cpu.vcpu, data); break;
    }
}
    ...
    if(id > 0 && id <= PLIC_IMPL_INTERRUPTS && !vplic_get_pend(vcpu, id)) {
        ...
        if(vplic_get_hw(vcpu, id)) {
            ...
            for(int i = 0; i < vplic->cntxt_num; i++) {
                if(plic_plat_id_to_cntxt(i).mode != PRIV_S)
                    continue;
                if(vplic_get_enbl(vcpu, i, id) && vplic_get_prio(vcpu, id) > vplic_get_theshold(vcpu, i)) {
                    vplic_update_hart_line(vcpu, i);
                }
            }
        }
...
static bool vplic_global_emul_handler(emul_access_t* acc) {
    ...
    switch((acc->addr >> 12) & 0x3) {
        case 0: vplic_emul_prio_access(acc); break;
        case 1: vplic_emul_pend_access(acc); break;
        default: vplic_emul_enbl_access(acc); break;
    }
    return true;
}
...
void vplic_init(vm_t* vm, uintptr_t vplic_base) {
    if(cpu.id == vm->master) {
        ...
        emul_mem_t plic_claimcomplte_emu = {.va_base = vplic_base + PLIC_CLAIMCMPLT_OFF,
