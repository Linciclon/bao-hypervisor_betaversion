...
void vm_arch_init(vm_t* vm, const vm_config_t* config) {
    ...
    unsigned long hgatp =
        (root_pt_pa >> PAGE_SHIFT) | (HGATP_MODE_DFLT) | ((vm->id << HGATP_VMID_OFF) & HGATP_VMID_MSK);
...
#define VM_LOAD(width, d, s)                                                                                           \
    ({                                                                                                                 \
        switch(width) {                                                                                                \
            case 1: *((uint8_t*)d) = hlvbu(s); break;                                                                  \
            case 2: *((uint16_t*)d) = hlvhu(s); break;                                                                 \
            case 4: *((uint32_t*)d) = hlvwu(s); break;                                                                 \
            case 8:                                                                                                    \
                *((uint64_t*)d) = hlvd(s);                                                                             \
                break;                                                                                                 \
...
#define VM_LOAD_EXEC(width, d, s)                                                                                      \
    ({                                                                                                                 \
        switch(width) {                                                                                                \
            case 2: *((uint16_t*)d) = hlvxhu(s); break;                                                                \
            case 4:                                                                                                    \
                *((uint32_t*)d) = hlvxwu(s);                                                                           \
                break;                                                                                                 \
                /*default: WARNING("trying to perform unsoported vm load")*/                                           \
