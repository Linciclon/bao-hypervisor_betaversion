...
void               pt_set_recursive(page_table_t* pt, uint64_t index) {
    ...
    pt->root_flags |= (index << PT_ROOT_FLAGS_REC_IND_OFF) & PT_ROOT_FLAGS_REC_IND_MSK;
}
void pt_arch_init() {
...
pte_t* pt_get_pte(page_table_t* pt, uint64_t lvl, void* va) {
    ...
    uint64_t      rec_ind = ((pt->root_flags & PT_ROOT_FLAGS_REC_IND_MSK) >> PT_ROOT_FLAGS_REC_IND_OFF);
    uint64_t      addr = ~mask;
    addr &= PTE_ADDR_MSK;
    addr &= ~(rec_ind_mask);
    addr |= ((rec_ind << rec_ind_off) & rec_ind_mask);
    addr |= (((((uint64_t)va) >> pt->dscr->lvl_off[lvl]) * sizeof(pte_t)) & (mask));
    return (pte_t*)addr;
}
