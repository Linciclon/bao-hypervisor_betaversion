...
void cache_arch_enumerate(cache_t* dscrp) {
    ...
    for(int i = 0; i < CLIDR_CTYPE_NUM; i++) {
        if((temp = bit_extract(clidr, i * CLIDR_CTYPE_LEN, CLIDR_CTYPE_LEN)) != 0) {
            dscrp->lvls++;
            switch(temp) {
                case CLIDR_CTYPE_IO: dscrp->type[i] = INSTRUCTION; break;
                case CLIDR_CTYPE_DO: dscrp->type[i] = DATA; break;
                case CLIDR_CTYPE_SP: dscrp->type[i] = SEPARATE; break;
                case CLIDR_CTYPE_UN: dscrp->type[i] = UNIFIED; break;
            }
        } else {
            break;
        }
    ...
    for(int lvl = 0; lvl < dscrp->lvls; lvl++) {
        ...
        if(dscrp->type[lvl] != INSTRUCTION) {
            ...
            dscrp->line_size[lvl][0] = 1UL << (bit_extract(ccsidr, CCSIDR_LINESIZE_OFF, CCSIDR_LINESIZE_LEN) + 4);
            dscrp->assoc[lvl][0] = bit_extract(ccsidr, CCSIDR_ASSOCIATIVITY_OFF, CCSIDR_ASSOCIATIVITY_LEN) + 1;
            dscrp->numset[lvl][0] = bit_extract(ccsidr, CCSIDR_NUMSETS_OFF, CCSIDR_NUMSETS_LEN) + 1;
        ...
        if(dscrp->type[lvl] == SEPARATE || dscrp->type[lvl] == INSTRUCTION) {
            ...
            MSR(CSSELR_EL1, csselr);
            ccsidr = MRS(CCSIDR_EL1);
            dscrp->line_size[lvl][1] = 1UL << (bit_extract(ccsidr, CCSIDR_LINESIZE_OFF, CCSIDR_LINESIZE_LEN) + 4);
            dscrp->assoc[lvl][1] = bit_extract(ccsidr, CCSIDR_ASSOCIATIVITY_OFF, CCSIDR_ASSOCIATIVITY_LEN) + 1;
            dscrp->numset[lvl][1] = bit_extract(ccsidr, CCSIDR_NUMSETS_OFF, CCSIDR_NUMSETS_LEN) + 1;
            ctr = MRS(CTR_EL0);
...
void cache_flush_range(void* base, uint64_t size) {
    ...
    uint64_t min_line_size = 1UL << bit_extract(ctr, CTR_DMINLINE_OFF, CTR_DMINLINE_LEN);
