...
void psci_cpumsg_handler(uint32_t event, uint64_t data) {
    switch(event) {
        case PSCI_MSG_ON: psci_wake_from_off(); break;
    }
}
CPU_MSG_HANDLER(psci_cpumsg_handler, PSCI_CPUSMG_ID);
int64_t psci_cpu_suspend_handler(uint64_t power_state, uintptr_t entrypoint, uint64_t context_id) {
    /**
...
int64_t psci_cpu_on_handler(uint64_t target_cpu, uintptr_t entrypoint, uint64_t context_id) {
    int64_t ret;
    vm_t*   vm = cpu.vcpu->vm;
...
int64_t psci_affinity_info_handler(uint64_t target_affinity, uint64_t lowest_affinity_level) {
...
int64_t psci_features_handler(uint64_t feature_id) {
    ...
    switch(feature_id) {
        ...
        case PSCI_FEATURES: ret = PSCI_E_SUCCESS; break;
    }
    return ret;
}
int64_t psci_smc_handler(uint32_t smc_fid, uint64_t x1, uint64_t x2, uint64_t x3) {
    int64_t ret = PSCI_E_NOT_SUPPORTED;
    switch(smc_fid) {
        case PSCI_VERSION: ret = psci_version(); break;
        case PSCI_CPU_OFF: ret = psci_cpu_off_handler(); break;
        case PSCI_CPU_SUSPEND_AARCH64: ret = psci_cpu_suspend_handler(x1, x2, x3); break;
        case PSCI_CPU_ON_AARCH64: ret = psci_cpu_on_handler(x1, x2, x3); break;
        case PSCI_AFFINITY_INFO_AARCH64: ret = psci_affinity_info_handler(x1, x2); break;
        case PSCI_FEATURES: ret = psci_features_handler(x1); break;
        case PSCI_MIG_INFO_TYPE: ret = PSCI_TOS_NOT_PRESENT_MP; break;
        default: INFO("unkown psci smc_fid 0x%lx", smc_fid);
    }
    return ret;
}
...
uint64_t psci_cpu_suspend(uint64_t power_state, uintptr_t entrypoint, uint64_t context_id) {
    return smc_call(PSCI_CPU_SUSPEND_AARCH64, power_state, entrypoint, context_id, NULL);
}
uint64_t psci_cpu_off(void) {
    return smc_call(PSCI_CPU_OFF, 0, 0, 0, NULL);
}
uint64_t psci_cpu_on(uint64_t target_cpu, uintptr_t entrypoint, uint64_t context_id) {
    return smc_call(PSCI_CPU_ON_AARCH64, target_cpu, entrypoint, context_id, NULL);
}
uint64_t psci_affinity_info(uint64_t target_affinity, uint64_t lowest_affinity_level) {
    return smc_call(PSCI_AFFINITY_INFO_AARCH64, target_affinity, lowest_affinity_level, 0, NULL);
}
