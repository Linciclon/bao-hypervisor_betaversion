...
#define GICR_IS_REG(REG, offset)                                                                                       \
    (((offset) >= offsetof(gicr_t, REG)) && (offset) < (offsetof(gicr_t, REG) + sizeof(gicr[0].REG)))
...
bool vgic_int_vcpu_is_target(vcpu_t* vcpu, vgic_int_t* interrupt) {
    ...
    bool routed_here = !priv && !(interrupt->phys.route ^ (MRS(MPIDR_EL1) & MPIDR_AFF_MSK));
...
void vgicr_emul_pidr_access(emul_access_t*                acc,
    ...
    if(!acc->write) {
        ...
        vcpu_writereg(cpu.vcpu, acc->reg, gicr[pgicr_id].ID[((acc->addr & 0xff) - 0xd0) / 4]);
    }
}
...
bool vgicr_emul_handler(emul_access_t* acc) {
    ...
    switch(GICR_REG_MASK(acc->addr)) {
        case GICR_REG_OFF(CTLR): handler_info = &vgicr_ctrl_info; break;
        case GICR_REG_OFF(TYPER): handler_info = &vgicr_typer_info; break;
        case GICR_REG_OFF(ISENABLER0): handler_info = &isenabler_info; break;
        case GICR_REG_OFF(ISPENDR0): handler_info = &ispendr_info; break;
        case GICR_REG_OFF(ISACTIVER0): handler_info = &iactiver_info; break;
        case GICR_REG_OFF(ICENABLER0): handler_info = &icenabler_info; break;
        case GICR_REG_OFF(ICPENDR0): handler_info = &icpendr_info; break;
        case GICR_REG_OFF(ICACTIVER0): handler_info = &icfgr_info; break;
        case GICR_REG_OFF(ICFGR0):
        case GICR_REG_OFF(ICFGR1): handler_info = &icfgr_info; break;
        default: {
    ...
    if(vgic_check_reg_alignment(acc, handler_info)) {
        ...
        vcpu_t*  vcpu = vgicr_id == cpu.vcpu->id ? cpu.vcpu : vm_get_vcpu(cpu.vcpu->vm, vgicr_id);
...
bool vgic_icc_sgir_handler(emul_access_t* acc) {
    if(acc->write) {
        ...
        if(sgir & ICC_SGIR_IRM_BIT) {
             ...
             */
            trgtlist = vm_translate_to_pcpu_mask(cpu.vcpu->vm, ICC_SGIR_TRGLSTFLT(sgir), cpu.vcpu->vm->cpu_num);
        }
...
void vgic_init(vm_t* vm, const struct gic_dscrp* gic_dscrp) {
    ...
    list_foreach(vm->vcpu_list, vcpu_t, vcpu) {
        ...
        emul_mem_t gicr_emu = {.va_base = gic_dscrp->gicr_addr + sizeof(gicr_t) * vcpu->id,
    ...
    emul_reg_t icc_sgir_emu = {.addr = SYSREG_ENC_ADDR(3, 0, 12, 11, 5), .handler = vgic_icc_sgir_handler};
    vm_emul_add_reg(vm, &icc_sgir_emu);
    emul_reg_t icc_sre_emu = {.addr = SYSREG_ENC_ADDR(3, 0, 12, 12, 5), .handler = vgic_icc_sre_handler};
