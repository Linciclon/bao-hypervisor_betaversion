...
void aborts_data_lower(uint32_t iss, uint64_t far, uint64_t il) {
    ...
    uint32_t DSFC = bit_extract(iss, ESR_ISS_DA_DSFC_OFF, ESR_ISS_DA_DSFC_LEN) & (0xf << 2);
    ...
    if(handler != NULL) {
        emul_access_t emul;
        emul.addr = addr;
        emul.width = (1 << bit_extract(iss, ESR_ISS_DA_SAS_OFF, ESR_ISS_DA_SAS_LEN));
        ...
        ERROR("no emulation handler for abort(0x%x at 0x%x)", far, cpu.vcpu->regs->elr_el2);
    }
}
...
void hvc64_handler(uint32_t iss, uint64_t far, uint64_t il) {
    ...
    switch(hvc_fid) {
        case HC_IPC: ret = ipc_hypercall(x1, x2, x3); break;
    }
    vcpu_writereg(cpu.vcpu, 0, ret);
}
void sysreg_handler(uint32_t iss, uint64_t far, uint64_t il) {
    ...
    if(handler != NULL) {
        ...
        ERROR("no emulation handler for register access (0x%x at 0x%x)", reg_addr, cpu.vcpu->regs->elr_el2);
    }
}
