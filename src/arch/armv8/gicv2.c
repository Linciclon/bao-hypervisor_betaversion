...
static inline void gicc_init() {
    ...
    int64_t  gic_cpu_id = bitmap_find_nth((bitmap_t)&sgi_targets, GIC_TARGET_BITS, 1, 0, true);
    if(gic_cpu_id < 0) {
...
void gic_map_mmio() {
    mem_map_dev(&cpu.as, (void*)&gicc, platform.arch.gic.gicc_addr, NUM_PAGES(sizeof(gicc)));
    mem_map_dev(&cpu.as, (void*)&gich, platform.arch.gic.gich_addr, NUM_PAGES(sizeof(gich)));
    mem_map_dev(&cpu.as, (void*)&gicd, platform.arch.gic.gicd_addr, NUM_PAGES(sizeof(gicd)));
}
uint32_t gicc_iar() {
    return gicc.IAR;
}
...
void gic_send_sgi(uint64_t cpu_target, uint64_t sgi_num) {
    if(sgi_num < GIC_MAX_SGIS && cpu_target < GIC_MAX_TARGETS) {
        gicd.SGIR = (1UL << (GICD_SGIR_CPUTRGLST_OFF + gic_cpu_map[cpu_target])) | (sgi_num & GICD_SGIR_SGIINTID_MSK);
    }
}
...
void gicd_set_trgt(uint64_t int_id, uint8_t cpu_targets) {
    ...
    gicd.ITARGETSR[reg_ind] =
        (gicd.ITARGETSR[reg_ind] & ~mask) | ((gic_translate_cpu_to_trgt(cpu_targets) << off) & mask);
