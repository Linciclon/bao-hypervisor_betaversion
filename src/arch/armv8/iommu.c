...
int iommu_arch_vm_init(vm_t* vm, const vm_config_t* config) {
    vm->iommu.arch.global_mask = config->platform.arch.smmu.global_mask | platform.arch.smmu.global_mask;
    ...
    for(int i = 0; i < config->platform.arch.smmu.group_num; i++) {
        /* Register each group. */
        const struct smmu_group* group = &config->platform.arch.smmu.smmu_groups[i];
