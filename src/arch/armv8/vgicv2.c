...
uint64_t vgicd_get_trgt(vcpu_t* vcpu, vgic_int_t* interrupt) {
    if(gic_is_priv(interrupt->id)) {
        ...
        return (uint8_t)vm_translate_to_vcpu_mask(vcpu->vm, interrupt->targets, GIC_TARGET_BITS);
    }
}
    ...
    if((acc->addr & 0xfff) == (((uint64_t)&gicd.SGIR) & 0xfff)) {
        if(acc->write) {
            ...
            switch(GICD_SGIR_TRGLSTFLT(val)) {
                case 0:
                    trgtlist = vm_translate_to_pcpu_mask(cpu.vcpu->vm, GICD_SGIR_CPUTRGLST(val), GIC_TARGET_BITS);
                    break;
                case 1: trgtlist = cpu.vcpu->vm->cpus & ~(1U << cpu.vcpu->phys_id); break;
                case 2: trgtlist = (1U << cpu.id); break;
                case 3: return;
            }
...
void vgic_init(vm_t* vm, const struct gic_dscrp* gic_dscrp) {
    ...
    void*  va = mem_alloc_vpage(&vm->as, SEC_VM_ANY, (void*)gic_dscrp->gicc_addr, n);
