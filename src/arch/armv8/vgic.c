...
#define GICD_IS_REG(REG, offset)                                                                                       \
    (((offset) >= offsetof(gicd_t, REG)) && (offset) < (offsetof(gicd_t, REG) + sizeof(gicd.REG)))
...
#define VGIC_MSG_DATA(VM_ID, VGICRID, INT_ID, REG, VAL)                                                                \
    (((uint64_t)(VM_ID) << 48) | (((uint64_t)(VGICRID)&0xffff) << 32) | (((INT_ID)&0x7fff) << 16) |                    \
     (((REG)&0xff) << 8) | ((VAL)&0xff))
...
static inline vgic_int_t* vgic_get_int(vcpu_t* vcpu, uint64_t int_id, uint64_t vgicr_id) {
    if(int_id < GIC_CPU_PRIV) {
        vcpu_t* target_vcpu = vgicr_id == vcpu->id ? vcpu : vm_get_vcpu(vcpu->vm, vgicr_id);
...
void vgic_yield_ownership(vcpu_t* vcpu, vgic_int_t* interrupt) {
    if((GIC_VERSION == GICV2 && gic_is_priv(interrupt->id)) || !vgic_owns(vcpu, interrupt) || interrupt->in_lr ||
...
void vgic_send_sgi_msg(vcpu_t* vcpu, uint64_t pcpu_mask, uint64_t int_id) {
    cpu_msg_t msg = {VGIC_IPI_ID, VGIC_INJECT, VGIC_MSG_DATA(cpu.vcpu->vm->id, 0, int_id, 0, cpu.vcpu->id)};
...
void vgic_route(vcpu_t* vcpu, vgic_int_t* interrupt) {
    ...
    if(!interrupt->in_lr && vgic_int_has_other_target(vcpu, interrupt)) {
        cpu_msg_t msg = {VGIC_IPI_ID, VGIC_ROUTE, VGIC_MSG_DATA(vcpu->vm->id, vcpu->id, interrupt->id, 0, 0)};
        vgic_yield_ownership(vcpu, interrupt);
        uint64_t trgtlist = vgic_int_ptarget_mask(vcpu, interrupt) & ~(1ull << vcpu->phys_id);
        for(int i = 0; i < platform.cpu_num; i++) {
            if(trgtlist & (1ull << i)) {
                cpu_send_msg(i, &msg);
            }
...
static inline void vgic_write_lr(vcpu_t* vcpu, vgic_int_t* interrupt, uint64_t lr_ind) {
    ...
    if((prev_int_id != interrupt->id) && !gic_is_priv(prev_int_id)) {
        ...
        if(prev_interrupt != NULL) {
            ...
            if(vgic_owns(vcpu, prev_interrupt) && prev_interrupt->in_lr && (prev_interrupt->lr == lr_ind)) {
...
#if(GIC_VERSION == GICV2)
    lr |= ((uint64_t)(interrupt->prio >> 3) << GICH_LR_PRIO_OFF) & GICH_LR_PRIO_MSK;
#else
    lr |= (((uint64_t)interrupt->prio << GICH_LR_PRIO_OFF) & GICH_LR_PRIO_MSK) | GICH_LR_GRP_BIT;
#endif
...
bool vgic_add_lr(vcpu_t* vcpu, vgic_int_t* interrupt) {
    ...
    if(lr_ind < 0) {
        ...
        if(lr_ind >= 0) {
            vgic_int_t* spilled_int = vgic_get_int(vcpu, GICH_LR_VID(gich_read_lr(lr_ind)), vcpu->id);
            if(spilled_int != NULL) {
        ...
        case GICD_REG_IND(CTLR):
            if(acc->write) {
                ...
                if(prev_ctrl ^ vgicd->CTLR) {
                    ...
                    cpu_msg_t msg = {VGIC_IPI_ID, VGIC_UPDATE_ENABLE, VGIC_MSG_DATA(cpu.vcpu->vm->id, 0, 0, 0, 0)};
...
void vgic_emul_razwi(emul_access_t* acc, struct vgic_reg_handler_info* handlers, bool gicr_access, uint64_t vgicr_id) {
    if(!acc->write)
        vcpu_writereg(cpu.vcpu, acc->reg, 0);
}
void vgic_int_set_field(struct vgic_reg_handler_info* handlers, vcpu_t* vcpu, vgic_int_t* interrupt, uint64_t data) {
...
void vgic_emul_generic_access(emul_access_t*                acc,
    ...
    uint64_t first_int = (GICD_REG_MASK(acc->addr) - handlers->regroup_base) * 8 / field_width;
    ...
    if(valid_access) {
        for(int i = 0; i < ((acc->width * 8) / field_width); i++) {
            vgic_int_t* interrupt = vgic_get_int(cpu.vcpu, first_int + i, vgicr_id);
            if(interrupt == NULL)
                break;
            if(acc->write) {
...
struct vgic_reg_handler_info* reg_handler_info_table[VGIC_REG_HANDLER_ID_NUM] = {[VGIC_ISENABLER_ID] = &isenabler_info,
...
bool vgic_check_reg_alignment(emul_access_t* acc, struct vgic_reg_handler_info* handlers) {
...
bool vgicd_emul_handler(emul_access_t* acc) {
    ...
    switch(GICD_REG_MASK(acc->addr) >> 7) {
        case GICD_REG_GROUP(CTLR): handler_info = &vgicd_misc_info; break;
        case GICD_REG_GROUP(ISENABLER): handler_info = &isenabler_info; break;
        case GICD_REG_GROUP(ISPENDR): handler_info = &ispendr_info; break;
        case GICD_REG_GROUP(ISACTIVER): handler_info = &isactiver_info; break;
        case GICD_REG_GROUP(ICENABLER): handler_info = &icenabler_info; break;
        case GICD_REG_GROUP(ICPENDR): handler_info = &icpendr_info; break;
        case GICD_REG_GROUP(ICACTIVER): handler_info = &iactiver_info; break;
        case GICD_REG_GROUP(ICFGR): handler_info = &icfgr_info; break;
        case GICD_REG_GROUP(SGIR): handler_info = &sgir_info; break;
        default: {
...
void vgic_ipi_handler(uint32_t event, uint64_t data) {
    ...
    switch(event) {
        ...
        case VGIC_ROUTE: {
            vgic_int_t* interrupt = vgic_get_int(cpu.vcpu, int_id, cpu.vcpu->id);
            if(interrupt != NULL) {
                spin_lock(&interrupt->lock);
...
void vgic_refill_lrs(vcpu_t* vcpu) {
    ...
    int64_t  lr_ind;
    uint64_t elrsr;
    while(elrsr = gich_get_elrsr(), (lr_ind = bitmap_find_nth((bitmap_t)&elrsr, NUM_LRS, 1, 0, true)) >= 0) {
        ...
        for(int i = 0; i < gic_num_irqs(); i++) {
            ...
            if(vgic_get_ownership(vcpu, temp_int)) {
                ...
                if(cpu_is_target && temp_state != INV && !temp_int->in_lr) {
                    ...
                    if((interrupt == NULL) || is_first_pend || is_act_after_pend || is_higher_prio) {
                        prev_pend = is_pend;
...
void vgic_handle_trapped_eoir(vcpu_t* vcpu) {
    int64_t  lr_ind = -1;
    uint64_t eisr = 0;
    while(eisr = gich_get_eisr(), (lr_ind = bitmap_find_nth((bitmap_t)&eisr, NUM_LRS, 1, 0, true)) >= 0) {
        uint64_t lr_val = gich_read_lr(lr_ind);
        gich_write_lr(lr_ind, 0);
        vgic_int_t* interrupt = vgic_get_int(vcpu, GICH_LR_VID(lr_val), vcpu->id);
        if(interrupt == NULL)
            continue;
        spin_lock(&interrupt->lock);
        interrupt->in_lr = false;
...
uint64_t vgic_get_itln(const struct gic_dscrp* gic_dscrp) {
     ...
     */
    uint64_t vtyper_itln = bit_extract(gicd.TYPER, GICD_TYPER_ITLN_OFF, GICD_TYPER_ITLN_LEN);
    if(gic_dscrp->interrupt_num > GIC_MAX_PPIS) {
        vtyper_itln = (ALIGN(gic_dscrp->interrupt_num, 32) / 32 - 1) & BIT_MASK(0, GICD_TYPER_ITLN_LEN);
    }
    return vtyper_itln;
}
