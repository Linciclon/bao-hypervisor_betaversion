...
void vmm_arch_init() {
    ...
    cpu_sync_barrier(&cpu_glb_sync);
    uint64_t vtcr = VTCR_RES1 | ((parange << VTCR_PS_OFF) & VTCR_PS_MSK) | VTCR_TG0_4K | VTCR_ORGN0_WB_RA_WA |
                    VTCR_IRGN0_WB_RA_WA | VTCR_T0SZ(64 - parange_table[parange]) | VTCR_SH0_IS |
                    ((parange_table[parange] < 44) ? VTCR_SL0_12 : VTCR_SL0_01);
    MSR(VTCR_EL2, vtcr);
    uint64_t hcr = HCR_VM_BIT | HCR_RW_BIT | HCR_IMO_BIT | HCR_FMO_BIT | HCR_TSC_BIT; /* trap smc */
    MSR(HCR_EL2, hcr);
}
