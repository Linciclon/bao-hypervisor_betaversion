...
void gicr_set_prio(uint64_t int_id, uint8_t prio, uint32_t gicr_id) {
    ...
    spin_lock(&gicr_lock);
    gicr[gicr_id].IPRIORITYR[reg_ind] = (gicr[gicr_id].IPRIORITYR[reg_ind] & ~mask) | ((prio << off) & mask);
...
uint64_t gicr_get_prio(uint64_t int_id, uint32_t gicr_id) {
    ...
    uint64_t off = GIC_PRIO_OFF(int_id);
    spin_lock(&gicr_lock);
    uint64_t prio = gicr[gicr_id].IPRIORITYR[reg_ind] >> off & BIT_MASK(off, GIC_PRIO_BITS);
    spin_unlock(&gicr_lock);
    return prio;
}
...
void gic_send_sgi(uint64_t cpu_target, uint64_t sgi_num) {
    if(sgi_num < GIC_MAX_SGIS) {
        ...
        uint64_t sgi = (MPIDR_AFF_LVL(mpidr, 1) << ICC_SGIR_AFF1_OFFSET) | (1UL << MPIDR_AFF_LVL(mpidr, 0)) |
