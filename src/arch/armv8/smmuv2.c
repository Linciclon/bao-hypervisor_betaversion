...
#define smmu_for_each_sme(sme)                                                                                         \
    for(int __bit = bitmap_get(smmu.sme_bitmap, sme); sme < smmu.sme_num; __bit = bitmap_get(smmu.sme_bitmap, ++sme))  \
        if(!__bit)                                                                                                     \
            continue;                                                                                                  \
        else
/**
...
static void smmu_check_features() {
    int version = bit_extract(smmu.hw.glbl_rs0->IDR7, SMMUV2_IDR7_MAJOR_OFF, SMMUV2_IDR7_MAJOR_LEN);
    if(version != 2) {
    ...
    int pasize = bit_extract(smmu.hw.glbl_rs0->IDR2, SMMUV2_IDR2_OAS_OFF, SMMUV2_IDR2_OAS_LEN);
    int ipasize = bit_extract(smmu.hw.glbl_rs0->IDR2, SMMUV2_IDR2_IAS_OFF, SMMUV2_IDR2_IAS_LEN);
    int parange = 0;
...
void smmu_init() {
     ...
     */
    smmu_glbl_rs0_t* smmu_glbl_rs0 = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, NULL, NUM_PAGES(sizeof(smmu_glbl_rs0_t)));
    mem_map_dev(&cpu.as, smmu_glbl_rs0, platform.arch.smmu.base, NUM_PAGES(sizeof(smmu_glbl_rs0_t)));
    uint32_t pg_size = smmu_glbl_rs0->IDR1 & SMMUV2_IDR1_PAGESIZE_BIT ? 0x10000 : 0x1000;
    uint32_t num_page =
        1ULL << (bit_extract(smmu_glbl_rs0->IDR1, SMMUV2_IDR1_NUMPAGEDXB_OFF, SMMUV2_IDR1_NUMPAGEDXB_LEN) + 1);
    int32_t          ctx_bank_num = bit_extract(smmu_glbl_rs0->IDR1, SMMUV2_IDR1_NUMCB_OFF, SMMUV2_IDR1_NUMCB_LEN);
    smmu_glbl_rs1_t* smmu_glbl_rs1 = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, NULL, NUM_PAGES(sizeof(smmu_glbl_rs1_t)));
    mem_map_dev(&cpu.as, smmu_glbl_rs1, platform.arch.smmu.base + pg_size, NUM_PAGES(sizeof(smmu_glbl_rs1_t)));
    smmu_cntxt_t* smmu_cntxt = mem_alloc_vpage(&cpu.as, SEC_HYP_GLOBAL, NULL, NUM_PAGES((pg_size * ctx_bank_num)));
    mem_map_dev(&cpu.as, smmu_cntxt, platform.arch.smmu.base + (num_page * pg_size), NUM_PAGES(pg_size * ctx_bank_num));
...
void smmu_write_ctxbnk(int32_t ctx_id, void* root_pt, uint32_t vm_id) {
    ...
    if(!bitmap_get(smmu.ctxbank_bitmap, ctx_id)) {
        ...
        smmu.hw.cntxt[ctx_id].TTBR0 = ((uint64_t)root_pt) & SMMUV2_CB_TTBA(smmu_cb_ttba_offset(t0sz));
 ...
 */
bool smmu_compatible_sme_exists(uint16_t mask, uint16_t id, uint32_t ctx, bool group) {
    bool     included = false;
    uint32_t sme = 0;
    spin_lock(&smmu.sme_lock);
